...
#include "zstd_internal.h"  /* ZSTD_v1_5_2_customMalloc, ZSTD_v1_5_2_customFree */
#include "pool.h"
...
struct POOL_ctx_s {
    ...
    ZSTD_v1_5_2_pthread_mutex_t queueMutex;
    /* Condition variable for pushers to wait on when the queue is full */
    ZSTD_v1_5_2_pthread_cond_t queuePushCond;
    /* Condition variables for poppers to wait on when the queue is empty */
    ZSTD_v1_5_2_pthread_cond_t queuePopCond;
...
static void* POOL_thread(void* opaque) {
    ...
    for (;;) {
        ...
        {   POOL_job const job = ctx->queue[ctx->queueHead];
            ...
            ZSTD_v1_5_2_pthread_cond_signal(&ctx->queuePushCond);
            ZSTD_v1_5_2_pthread_mutex_unlock(&ctx->queueMutex);
                               ...
                               ZSTD_v1_5_2_customMem customMem)
{
    ...
    ctx->threads = (ZSTD_v1_5_2_pthread_t*)ZSTD_v1_5_2_customMalloc(numThreads * sizeof(ZSTD_v1_5_2_pthread_t), customMem);
...
static void POOL_join(POOL_ctx* ctx) {
    ...
    ZSTD_v1_5_2_pthread_cond_broadcast(&ctx->queuePushCond);
    ZSTD_v1_5_2_pthread_cond_broadcast(&ctx->queuePopCond);
    /* Join all of the threads */
    {   size_t i;
        for (i = 0; i < ctx->threadCapacity; ++i) {
            ZSTD_v1_5_2_pthread_join(ctx->threads[i], NULL);  /* note : could fail */
    }   }
}
...
void POOL_free(POOL_ctx *ctx) {
    ...
    ZSTD_v1_5_2_customFree(ctx->queue, ctx->customMem);
    ZSTD_v1_5_2_customFree(ctx->threads, ctx->customMem);
    ZSTD_v1_5_2_customFree(ctx, ctx->customMem);
}
...
static int POOL_resize_internal(POOL_ctx* ctx, size_t numThreads)
{
    ...
    {   ZSTD_v1_5_2_pthread_t* const threadPool = (ZSTD_v1_5_2_pthread_t*)ZSTD_v1_5_2_customMalloc(numThreads * sizeof(ZSTD_v1_5_2_pthread_t), ctx->customMem);
        if (!threadPool) return 1;
        /* replace existing thread pool */
        ZSTD_v1_5_2_memcpy(threadPool, ctx->threads, ctx->threadCapacity * sizeof(*threadPool));
        ZSTD_v1_5_2_customFree(ctx->threads, ctx->customMem);
